...
export const processingExcelData = async (excelFile: File) => {
  ...
  const {graphProcessingData, stockPriceList, culmulativeList} = processingExcelDataForCummulativeGraph(filtered, baseDataBeforeProcess.cumulativeStockData);
...
//   console.log(cumulativeGraphData, cumulativeLastestData);
  ...
  callPostApi("/api/excel", cumulativeTableData);
};
  ...
  data.forEach((item, idx) => {
    ...
    const dayValue: ChartData = {
      ...
      세력합 : {
        ...
        dispersionRatio: calcPercent(volume.totalForeAndInstCollectionVolume, cumulativeStockData.maxTotalForeAndInstMount - cumulativeStockData.minTotalForeAndInstMount),
        stockMomentum: calcPercent(cumulativeStockData.maxTotalForeAndInstMount - cumulativeStockData.minTotalForeAndInstMount , sumTotalCollectionVolume),
      },
      외국인 : {
        ...defaultInfo,
    ...
    if(key === 'week') {
      ...
      stockList[key].forEach((data: originalExcelFile, idx: number) => {
        ...
        if(idx + 1 === stockList[key].length) {
          result.push(Object.assign({}, cumulativeData, { tradeDateNm: data.tradeDateNm, avgMount: Math.floor(cumulativeData.avgMount / stockList[key].length) }));
        }
      })
    }
  })
